...
from texts.terms import TERMS_ACCEPTED_TEXT, TERMS_TEXT

router = Router()

# In-memory store of users who accepted the terms.
...
async def accept_terms(callback_query: CallbackQuery) -> None:
    """Accept the user agreement and show the main menu."""
...
async def terms_back_to_menu(callback_query: CallbackQuery) -> None:
    """Return to the main menu from the user agreement screen."""
    if callback_query.message:
        await callback_query.message.edit_text(
            start_text(),
            reply_markup=main_menu_keyboard(),
