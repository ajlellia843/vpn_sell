...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="🔒 Получить VPN 🔒",
            callback_data=CommonMenuCallback(action="get_vpn").pack(),
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="📍 Доступные локации",
            callback_data=CommonMenuCallback(action="locations").pack(),
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="🙋 Пригласить",
            callback_data=ReferralActionCallback(action="referral").pack(),
        ),
        InlineKeyboardButton(
            text="❓ Помощь",
            callback_data=CommonMenuCallback(action="help").pack(),
        ),
    )
    builder.row(
        InlineKeyboardButton(
            text="🎁 Попробовать БЕСПЛАТНО",
            callback_data=CommonMenuCallback(action="trial").pack(),
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="Как установить",
            callback_data=CommonMenuCallback(action="how_install").pack(),
        )
    )
